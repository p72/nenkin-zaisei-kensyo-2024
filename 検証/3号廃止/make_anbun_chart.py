...
def main():
    ...
    fig, axes = plt.subplots(2, 3, figsize=(14.5, 11.6), sharey=True, sharex=True)
    ...
    note = "\n".join((
        ...
        "　　　　保険料は国民年金勘定に入るのに負担する頭数は増えないので、国民年金はすぐ黒字になる。",
        "",
        "結果　　基礎年金はマクロ経済スライドが不要（カット率の上限）に張りつく。均衡解ではなく天井。",
        "　　　　上がった基礎年金の費用は、大半を厚生年金と国庫が払う。過去30年投影では報酬比例が",
        "　　　　3.2ポイント下がり、国庫負担は2100年度で年7兆円増える。3号の保険料の6割は給付に使われず",
        "　　　　国民年金の積立金に積み上がる（国年積立金_按分据置.png）。3号の保険料そのものの効果は、",
        "　　　　調整期間の一致の下で比べると +1.4ポイント（過去30年投影）。",
        "",
        "非公式の独自計算。厚生労働省の試算ではない。",
    ))
    fig.text(0.012, 0.010, note, ha="left", va="bottom", fontsize=9.5,
             color=INK2, linespacing=1.55)
    fig.tight_layout(rect=(0, 0.30, 1, 0.955))
