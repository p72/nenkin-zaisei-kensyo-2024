#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""拠出金按分据置型で3号の保険料はどこへ行くか — 国民年金勘定の積立金

現行制度と「3号廃止・拠出金按分据置型」の2本を、経済前提2ケース
（過去30年投影・高成長実現）× 2つの物差しで並べる。

  左: 年度末積立金の現在価値（2024年度末、兆円）
  右: 積立度合（前年度末積立金 ÷ 当年度支出。仕様書 §8.1）

  検証/3号廃止/図/国年積立金_按分据置.png

**金額は現在価値で見る。** 積立金は運用利回りで増えるので、2024年度価格
（賃金で割り戻す。公表の慣行）で見ると、運用で膨らんだ分まで含んで
90年間で雪だるま式に大きく見える（過去30年投影の2119年度で約112兆円の差）。
④が使う名目運用利回り（kekka の「経済前提」ブロック）で割り引くと、
運用で膨らんだ分が消え、積み上がった保険料の正味（割引後の累計）だけが残る。
積立度合は比率なので物差しの選び方に左右されない。
...
def discount(case):
    """2024年度末を1とする割引係数 D[k]（④の名目運用利回り 1+i[k] で割り引く）"""
    kek = os.path.join(BAS, f"kekka{case}-{case}-{case}-{case}-1120-000a.csv")
    rate = _m.series(_m.read_blocks(kek)["経済前提"], "運用利回り")
    d, out = 1.0, {Y0: 1.0}
...
def load(case):
    ...
    for yobi, _l, _c in SCEN:
        ...
        out[yobi] = {
            "yrs": yrs,
            "pv": [nat[y]["年度末積立金"] * D[y] for y in yrs],
    ...
    # 3号の保険料の現在価値（積み上がった分と同じ物差しで比べる）
    out["prem_pv"] = sum((s[y]["保険料収入"] - b[y]["保険料収入"]) * D[y]
                         for y in range(JISSHI, Y1 + 1))
    out["gap_pv"] = out["501"]["pv"][-1] - out["000"]["pv"][-1]
    # 参考: 2024年度価格（賃金で割り戻し）での2119年度の差。運用で膨らんだ分を含む
    out["gap_24"] = (s[Y1]["年度末積立金"] - b[Y1]["年度末積立金"]) * kk[Y0] / kk[Y1]
    return out
...
def main():
    ...
    PANELS = (("pv", "年度末積立金（2024年度末の現在価値、兆円）", "{:.1f}兆円"),
    ...
    for row, (case, cname) in enumerate(CASES):
        ...
        for col, (key, ttl, fmt) in enumerate(PANELS):
            ...
            for yobi, label, color in SCEN:
                ...
                ax.annotate(fmt.format(v[-1]), xy=(yrs[-1], v[-1]), xytext=(5, 0),
        ...
        axes[row][0].annotate(
            f"2119年度の差 {d['gap_pv']:.1f}兆円（現在価値）\n"
            f"3号の保険料のうち、国民年金勘定が\n給付に使ったのは {d['share_used'] * 100:.0f}%",
            xy=(0.40, 0.60), xycoords="axes fraction", va="top", fontsize=10,
    ...
    note = "\n".join((
        "読み方  縦の点線が実施年度。左は年度末積立金を④の名目運用利回りで2024年度末まで割り引いた現在価値で、運用で膨らんだ分を",
        "　　　　除いた「積み上がった保険料」の正味。積み上がったのは実施直後の十数年で、その後は横ばい（運用で膨らむだけ）。",
        f"　　　　（参考：2024年度価格＝賃金で割り戻すと、運用で膨らんだ分を含んで2119年度の差は約{data['3003']['gap_24']:.0f}兆円に見える）",
        "　　　　右の破線は有限均衡の目標（2120年度に積立度合1年分）。同じ列の2ケースは縦軸を共有している。",
        "",
        "仕組み  基礎年金の給付水準は国民年金勘定の財政だけで決まる（2段階均衡、仕様書 §8.2）。按分据置型では3号の保険料が",
        "　　　　国民年金勘定に入るのに、拠出金を負担する頭数は増えないので、国民年金はすぐに黒字になる。基礎年金はマクロ経済",
        "　　　　スライドが不要（カット率の上限）に張りつき、それ以上は給付に使い道がない。",
        "",
    ...
    for case, cname in CASES:
        d = data[case]
        print(f"  {cname}: 2119年度の差 現在価値 {d['gap_pv']:.1f}兆円（2024年度価格では {d['gap_24']:.1f}兆円）、"
              f"給付に使った割合 {d['share_used']*100:.1f}%、積立度合 {d['501']['doai'][-1]:.2f}、"
              f"3号の保険料の現在価値 {d['prem_pv']:.1f}兆円（積み上がった分はその {d['gap_pv']/d['prem_pv']*100:.0f}%）")
