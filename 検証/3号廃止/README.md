...
（過去30年投影：[PNG](図/財政見通し_按分据置_3003.png)・[PDF](図/財政見通し_按分据置_3003.pdf)、
高成長実現：[PNG](図/財政見通し_按分据置_3001.png)・[PDF](図/財政見通し_按分据置_3001.pdf)）。
現行制度の行は公表の財政見通しと一致する。
`python3 検証/3号廃止/make_zaisei_hyou_sango.py` で作り直せる（ヘッドレス Chromium が要る）。
...
| `財政見通し_按分据置_3003.png` `_3001.png`（と `.pdf`） | **表**。厚生労働省の「財政見通し」と同じ形で、厚生年金・国民年金を現行制度と按分据置型の2行ずつ並べた1枚（A4縦）。現行と違う値は太字。`make_zaisei_hyou_sango.py` |
