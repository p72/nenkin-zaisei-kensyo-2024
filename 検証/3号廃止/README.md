...
> **読む前に。** ここで動かしているのは号別の頭数だけで、**定額保険料の水準・
> 厚生年金保険料率・勘定の壁の3つは現行のまま固定**している。そのため以下の
> 結果の大部分は、政策の含意というより**固定したパラメータの算術的な帰結**である。
>
> | 固定したもの | 値 | 何を強制しているか |
> |---|---|---|
> | 定額保険料 | 月17,587円（2027年度・実効） | 基礎年金の1人当たり実負担 21,037円 を下回る。頭数を移すほど赤字が増えるのは算術 |
> | 厚生年金保険料率 | 18.3%（据え置き） | 定額保険料が純粋な上乗せになる。4.5節の +10.8pt はこの負担増で買ったもの |
> | 勘定の壁 | 基礎の水準は国年勘定だけで決まる | 損は全受給者へ、得は厚年の積立金へ、の非対称が自動的に出る |
>
> 特に1つ目が効く。月17,000円（2004年度価格）は第1号が約600万人であることを
> 前提に決まった水準で、最初から1人当たり赤字であり、それを積立金の運用収入と
> マクロ経済スライドで埋めている。頭数を9倍にすれば赤字も9倍になる。
> **構造を動かさない限り、この試算は「いくら足りないか」しか語らない。**
> 動かす価値のある構造変数は 6. の末尾に整理した。
...
上の3つの固定を外さない限り答えは算術で決まる。外す候補は4つある。

| 案 | 内容 | 答えが事前に決まるか |
|---|---|---|
| a | 定額保険料を収支中立水準へ引き上げ | 決まる（当然均衡する）。ただし必要な水準の提示に価値 |
| b | 18.3% を負担中立まで引き下げ | 決まる。国年の収支は1円も変わらず、厚年の余剰が消えるだけ |
| **c = a + b** | 基礎年金を完全に定額拠出へ、総負担は中立 | **決まらない。分配の問題になる** |
| d | 国庫負担割合を 1/2 から引き上げ（税方式へ） | 決まるが、比較軸として明快 |

収支中立に必要な定額保険料と、負担中立の保険料率（過去30年投影ケース）。

| 年度 | 定額保険料（現行 → 収支中立） | 18.3% からの引下げ幅 |
|---|---|---|
| 2027 | 17,587円 → 21,037円 | 4.94pt（18.3% → 13.4%） |
| 2040 | 19,927円 → 27,856円 | 5.28pt |
| 2060 | 25,853円 → 33,879円 | 4.99pt |
| 2080 | 33,465円 → 46,297円 | 5.22pt |

案 c の収入は2027年度で 13.4兆円 となり、現行の拠出金実負担と第1号保険料の合計
13.2兆円 とほぼ一致する。総額では中立で、残るのは**誰が払うか**だけになる。

現行の拠出金制度は実質「基礎年金を報酬比例で賄っている」ので、案 c はそれを
定額拠出に切り替える実験になる。粗い試算では月収20万円の被用者が月 1.1万円 の
負担増、月収60万円の被用者が月 0.9万円 の負担減で、**逆進化の定量化**が
主結果になり得る。

実装は保険料水準のレバー（④が読む `hoken17000.csv`）と保険料率のレバー
（⑤ `shus.c:142` の `MIN(0.183, …)`）の2本。どちらも未実装。
