...
| 拠出金按分据置型 | 保険料は徴収するが、3号分の基礎年金費用は引き続き被用者年金が負担 | `SANGO_MODE=1` |
...
「保険料を払う1号」に移す頭数を増やすほど**国民年金勘定は赤字を積み増します**。
段階Aで積立金の枯渇は2054年度、段階Bでは2033年度になります。

**同じ政策が、増収をどの勘定に落とすかで符号ごと変わります。**

| 過去30年投影ケース | 所得代替率 | うち基礎 |
|---|---|---|
| 現行制度 | 50.39% | 25.52% |
| 第1号移行型（拠出金の按分も移す） | 36.12%（均衡せず） | 11.15% |
| 拠出金按分据置型（按分は現行のまま） | **57.94%（基礎は調整不要）** | 36.21% |

3号廃止で被用者年金に生まれる「3.1兆円の黒字」はグロスで、国庫負担の減
1.56兆円を差し引いた正味は1.47兆円です。公的年金全体の純増は元3号が払う
1.23兆円だけで、残りは配分の付け替えです。第1号移行型はこの純増を被用者年金の
積立金へ流してしまい、拠出金按分据置型は基礎年金に残します。差は21.8ポイントで、
追加の財源を1円も要しません。
...
報酬比例はどのシナリオでも動きません（調整がほぼ終わっているため）。国庫負担の
総額も、振替では直接には動きません（配分が変わるだけ）。なお段階Bの +10.8pt は、
第2号が18.3%に加えて定額保険料を払う年9.4兆円の負担増で買ったものです。
