...
│   └── 所得代替率_8ケース.png
├── 3号廃止/               原本にないレバー: 第3号被保険者の廃止（第1号への振替）
│   ├── run_sango.sh         ベース・3号廃止・調整期間一致の組合せを一式流す
...
最大相対差は `1.8e-14` で、倍精度の丸めそのものです。詳細は仕様書 §15.0.2。
...
| `SANGO` `SANGO_NOUFU` | **原本にない追加レバー**: 第3号被保険者の廃止（第1号への振替）の実施年度と振替後の納付率。公表値との照合対象ではない（下記） |
...
### 第3号被保険者の廃止 — 原本にないレバーで「もし」を計算する

公表されたオプション試算には無い政策（第3号被保険者を廃止して第1号にし、全員が
保険料を納める）を、④基礎年金の頭割りに29行のパッチを当てて計算しました。
厚労省の試算ではなく、このリポジトリの独自計算です。

```bash
検証/実行/run_pipeline.sh 3003 1 1 0 2                              # 通常試算（先に通す）
STEPS=45 YOBI=301 SANGO=2027 検証/実行/run_pipeline.sh 3003 1 1 0 2  # 3号廃止（2027年度）
検証/3号廃止/run_sango.sh && python3 検証/3号廃止/analyze_sango.py  # 一式＋比較表・図
```

結果と解釈は **[検証/3号廃止/README.md](検証/3号廃止/README.md)**、数表は
`検証/3号廃止/結果.md`。要点だけ書くと、第1号の保険料（月17,000円×改定率）が
基礎年金の1人当たり実負担（拠出金単価−国庫負担単価）を下回っているため、
3号を「保険料を払う1号」にすると**国民年金勘定は1人当たりで赤字を積み増し**、
過去30年投影ケースでは基礎年金の調整が2120年度まで終わりません。一方、
厚生年金勘定は拠出金が年3兆円減って大幅に改善しますが、報酬比例の調整は
ほぼ終わっているので受給者の年金は増えません。**マクロ経済スライドの調整期間を
一致させる場合だけ**、この改善が基礎年金にも回って所得代替率が上がります。
...
所得代替率も 56.9122495807027 まで全桁一致でした。この乗率が引かれるのは
